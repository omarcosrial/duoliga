...
function moreView(){
  ...
  return `<section class="view ${currentView==='more'?'active':''}">
    <div class="card app-card">
      ...
      <div class="actions-row"><button class="ghost-btn" id="choose-photo">📷 Alterar foto</button><button class="ghost-btn" id="change-user">Trocar usuário</button></div>
    ...
    <div class="card app-card">
      ...
      <div class="sep"></div>
      <form id="add-member-form" class="actions-row"><input class="input" style="flex:1" id="new-member" placeholder="Adicionar participante"><button class="ghost-btn">+ Adicionar</button></form>
    </div>
  </section>`;
}
...
function bind(){
  ...
  document.querySelector('#add-member-form')?.addEventListener('submit',async e=>{ e.preventDefault(); const name=document.querySelector('#new-member').value.trim(); if(!name) return; try{ const before = state.members.length; await action('addMember',{name}); const newest = state.members[state.members.length-1]; if(state.members.length===before+1 && newest){ currentUserId = newest.id; localStorage.setItem('duoliga-member', currentUserId); } toast(`${name} entrou no desafio!`,'success'); }catch(err){ toast(err.message,'error'); } });
}
...
function showOnboarding(){
  ...
  wrap.querySelectorAll('[data-member]').forEach(b=>b.addEventListener('click',()=>{ currentUserId=b.dataset.member; localStorage.setItem('duoliga-member',currentUserId); wrap.remove(); render(); }));
